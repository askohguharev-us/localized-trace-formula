...
\[
  k(\gamma z,\gamma w) = k(z,w), \qquad \forall \gamma\in \mathrm{PSL}_{2}(\mathbb{R}).
\]
...
\end{equation}
Thus $K_{Y}$ is supported on the truncated surface $M(Y)$ (up to the smoothing scale),
...
\begin{itemize}
  ...
  \item \emph{Local support:} For fixed $z$, the support of $K_{Y}(z,\cdot)$ is contained in a union of balls of radius $\mathrm{supp}(q)$. 
...
\[
  ...
  t\tanh(\pi t)\,dt \cdot \Lambda^{Y}_{\mathrm{sm}}(z)\Lambda^{Y}_{\mathrm{sm}}(w),
\]
with truncation restricting the effective range of $z,w$ to $M(Y)$.
...
if $\mathrm{supp}(q)\subset[0,R]$, then only $\gamma$ with $d(z,\gamma w)\le R$ matter.
...
\begin{itemize}
  \item To Chapter~2: all constants depend explicitly on $\Gamma$, cusp widths, and the spectral gap. 
...
\[
  \|K_{Y}\|_{L^{2}\to L^{2}}^{2}
  \le \left(\sup_{z\in M(Y)}\int_{M(Y)} |K_{Y}(z,w)|\,d\mu(w)\right)\,
       \left(\sup_{w\in M(Y)}\int_{M(Y)} |K_{Y}(z,w)|\,d\mu(z)\right).
\]
...
\[
  ...
  + \mathrm{Err}(Y),
\]
where $\mathrm{Err}(Y)\ll Y^{-1}$ from truncation.
...
Then
\[
  \mathrm{supp}(K_{Y}(z,\cdot)) \subset B(z,R)/\Gamma,
\]
...
\[
  K_{Y}(z,w)\neq 0 \;\Rightarrow\; \exists \gamma \text{ s.t. } d(z,\gamma w)\le R.
\]
Moreover, the effective diameter of the support satisfies
\[
  \mathrm{diam}(\mathrm{supp}(K_{Y})) \le R.
\]
\end{lemma}
...
and there are $O(e^{R})$ such $\gamma$ by the lattice point theorem.
\end{proof}
...
\[
  (K_{Y}f)(z) = \int_{M} K_{Y}(z,w)f(w)\,d\mu(w).
\]
Due to the support of $K_Y$, the integral is effectively over $B(z,R)/\Gamma$.
...
\[
  \mathrm{vol}(B(z,R)) = 2\pi(\cosh R - 1).
\]
...
Since $z,w\in M(Y)$, both coordinates satisfy $\mathrm{Im}(z),\mathrm{Im}(w)\le Y$ (in cusp coordinates).
If $d(z,w)\le R$, then
\[
  \max(\mathrm{Im}(z),\mathrm{Im}(w)) \le e^{R}Y.
\]
...
up to enlargement by a factor $e^{R}$ near the boundary.
\end{lemma}
...
which enlarges by at most $e^{R}$ in hyperbolic distance coordinates.
\end{proof}
...
\]
is an operator localized to $d(z,w)\le \rho$,
...
and inversely on $\mathrm{inj}(M(Y))$.
\end{lemma}

\begin{proof}
By local finiteness: only $O(e^{R}/\mathrm{inj}(M(Y))^{2})$ orbit points contribute,
...
\begin{proof}
The kernel is supported in a strip near the diagonal $\{d(z,\gamma w)\le R\}$,
with effective volume $O_{\Gamma}(e^{R})$,
and values bounded by $\|q\|_{\infty}$.
\end{proof}
...
\begin{proof}
$K_{Y}$ is smooth and compactly supported (in terms of $q$),
...
The difference $K(z,w)-K_{Y}(z,w)$ is supported in cusp regions with $y>Y$ (or near the boundary).
...
\begin{itemize}
  \item[(I1)] \emph{Dependence on data:} All constants are stated to depend only on $\Gamma$, cusp widths, the support radius $R$, and the injectivity radius $\mathrm{inj}(M(Y))$.
