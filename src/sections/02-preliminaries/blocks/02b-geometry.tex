...
\]
For $z, w \in \mathbb{H}$ the distance $d(z,w)$ is determined by the identity
\[
  \cosh d(z,w) = 1 + \frac{|z - w|^{2}}{2\,\mathrm{Im}(z)\,\mathrm{Im}(w)}.
\]
The Riemannian volume of a measurable set $\Omega \subset \mathbb{H}$ is
$\mathrm{vol}(\Omega) = \int_{\Omega} d\mu$.

\medskip

\noindent\textbf{Isometries.}
The group $\mathrm{PSL}_{2}(\mathbb{R})$ acts by fractional linear transformations
$\gamma z = \dfrac{az + b}{cz + d}$ with $\gamma = \begin{pmatrix} a & b \\ c & d \end{pmatrix}$
and preserves both $ds^{2}$ and $d\mu$.
Elements are classified by the absolute value of their trace $|\mathrm{tr}(\gamma)|$:
elliptic ($< 2$), parabolic ($= 2$), and hyperbolic ($> 2$).
Two hyperbolic elements are conjugate if and only if they have the same translation length,
defined below.
...
Geodesics in $\mathbb{H}$ are vertical lines and semicircles orthogonal to the real axis $\mathbb{R}$.
Let $\gamma \in \mathrm{PSL}_{2}(\mathbb{R})$ be a hyperbolic element.
...
A hyperbolic ball of radius $R$ has area
\[
  \mathrm{vol}(B(R)) = 2\pi(\cosh R - 1).
\]
Hence $\mathrm{vol}(B(R)) \sim \pi e^{R}$ as $R \to \infty$.
This exponential growth permeates all orbit and lattice point estimates.

\medskip

\noindent\textbf{Fundamental and Dirichlet domains.}
Let $\Gamma \subset \mathrm{PSL}_{2}(\mathbb{R})$ be a discrete group of finite covolume.
...
\[
  \mathcal{D}(z_{0}) = \{z \in \mathbb{H} : d(z, z_{0}) \le d(z, \gamma z_{0}) \text{ for all } \gamma \in \Gamma\}.
\]
It is convex, locally finite, and $\mathrm{vol}(\Gamma \backslash \mathbb{H}) = \int_{\mathcal{D}(z_{0})} d\mu$.
...
Then
\[
  N_{\Gamma}(R; z_{0}) = \frac{\mathrm{vol}(B(R))}{\mathrm{vol}(\Gamma \backslash \mathbb{H})} + O_{\Gamma}\!\left(\frac{e^{R}}{R^{2}}\right),
\]
uniformly as $R \to \infty$. The implied constant depends only on $\Gamma$.

\medskip

\noindent\textbf{Separation lemma (Selberg).}
There exists $\epsilon_{\Gamma} > 0$ such that
$d(\gamma\mathcal{F}, \gamma'\mathcal{F}) \ge \epsilon_{\Gamma}$ for any distinct $\gamma, \gamma' \in \Gamma$.
...
The resolvent kernel $G_{s}(z, w)$ on $\mathbb{H}$ solves the equation
\[
  (\Delta_{z} - s(1 - s))G_{s}(z, w) = -\delta_{w}(z),
\]
with an explicit expression via the hypergeometric function of $\cosh d(z, w)$. On quotients,
...
There exists a scaling matrix $\sigma_{\mathfrak{a}} \in \mathrm{PSL}_{2}(\mathbb{R})$ such that
...
$N = \{ \begin{pmatrix} 1 & t \\ 0 & 1 \end{pmatrix} : t \in \mathbb{R} \}$ and right multiplication by $\{\pm I\}$.
...
\[
  \mathrm{vol}(M \setminus M(Y)) = \sum_{\mathfrak{a}} \frac{w_{\mathfrak{a}}}{Y},
  \qquad
  \mathrm{length}(\partial M(Y)) = \sum_{\mathfrak{a}} \frac{w_{\mathfrak{a}}}{Y}.
\]
...
\[
  \mathrm{inj}(z) \ge c \cdot \min\{1, Y^{-1}\}.
\]
\end{lemma}
...
Hence $\mathrm{inj}(z) \asymp \min\{1, w_{\mathfrak{a}}/y\}$; for $y \ge Y$, this gives the stated bound with
$c = \frac{1}{2} \min_{\mathfrak{a}} w_{\mathfrak{a}}$.
\end{proof}
...
\[
  M_{\ge \epsilon} = \{z \in M : \mathrm{inj}(z) \ge \epsilon\}, \qquad
...
\[
  ...
  w(\ell) = \mathrm{arcsinh}\!\left(\frac{1}{\sinh(\ell/2)}\right),
\]
...
$\lambda = \frac{1}{4} + t^{2}$. This normalization matches the Plancherel measure
...
For $s \in \mathbb{C}$ with $\mathrm{Re}(s) > 1/2$, define
\[
  R(s) = (\Delta - s(1 - s))^{-1}.
\]
On $\mathbb{H}$
...
The geodesic flow on $T^{1}M$ is Anosov; it is mixing with respect to the Liouville measure.
...
Then
\[
  \mathrm{vol}(M) = 2\pi\left(2g - 2 + n + \sum_{j}(1 - 1/e_{j})\right).
\]
In particular, for torsion-free $\Gamma$ one has $\mathrm{vol}(M) = 2\pi(2g - 2 + n)$.
...
\begin{itemize}
  ...
  \item \emph{Chapter 4 (Spectral projector).} Lower bounds for $\mathrm{inj}$ and Sobolev constants yield
...
\begin{itemize}
  \item Distances, isometries, and classification agree with $\mathrm{PSL}_{2}(\mathbb{R})$ conventions used in the Selberg transform.
  ...
  \item Exact identities:
        $\mathrm{vol}(\pi(C_{\mathfrak{a}}) \setminus \pi(C_{\mathfrak{a}}(Y))) = w_{\mathfrak{a}}/Y$
        and $\mathrm{length}(\partial \pi(C_{\mathfrak{a}}(Y))) = w_{\mathfrak{a}}/Y$
