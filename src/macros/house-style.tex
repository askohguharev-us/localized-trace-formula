% House style (clean, no external includes)
% Requires: amsmath, amssymb, mathtools already loaded in main.tex

% ----- Small helpers -----
\newcommand{\abs}[1]{\left\lvert #1\right\rvert}
\newcommand{\norm}[1]{\left\lVert #1\right\rVert}
\newcommand{\ip}[2]{\left\langle #1,\,#2\right\rangle}
\newcommand{\set}[1]{\left\{\,#1\,\right\}}
\newcommand{\dd}{\mathrm{d}}
\newcommand{\eps}{\varepsilon}
\newcommand{\OO}{\mathcal{O}}
\newcommand{\oo}{o}

% Number sets
\newcommand{\R}{\mathbb{R}}
\newcommand{\C}{\mathbb{C}}
\newcommand{\Z}{\mathbb{Z}}
\newcommand{\N}{\mathbb{N}}
\newcommand{\Hh}{\mathbb{H}}

% ----- Operators (safe declarations) -----
\providecommand{\Op}{\operatorname{Op}} % do not error if defined elsewhere
\providecommand{\Opw}{\operatorname{Op}_h^{\mathrm{w}}}
\providecommand{\Opl}{\operatorname{Op}_h^{\mathrm{left}}}
\providecommand{\Opr}{\operatorname{Op}_h^{\mathrm{right}}}

\DeclareMathOperator{\sgn}{sgn}
\DeclareMathOperator{\tr}{tr}
\DeclareMathOperator{\Tr}{Tr}
\DeclareMathOperator{\diag}{diag}
\DeclareMathOperator{\supp}{supp}
\DeclareMathOperator{\rank}{rank}
\DeclareMathOperator{\dist}{dist}
\DeclareMathOperator{\vol}{vol}
\DeclareMathOperator{\Res}{Res}

% ----- TODO helper (can be disabled by comment) -----
\newcommand{\TODO}[1]{\textcolor{red!70!black}{\textbf{TODO:}~#1}}

% No external \input here. Keep this file self-contained.
