...
We work semiclassically with parameter $h \in (0, 1]$; when localization to frequency $\lambda \gg 1$ is intended, one may take $h \sim \lambda^{-1}$. Phase–space norms and trace identities are written at the $h$–scale; no substitution $h=1$ is performed implicitly.
...
is $\asymp W/Y$ by Lemma~A.\ref{lem:smooth-vol}. Combine Theorem~B.\ref{thm:B.8-trace}
with Hilbert–Schmidt estimates from Lemma~B.\ref{lem:B-HS}. Bound by Sobolev
...
\begin{proof}
Apply Theorem~B.\ref{thm:B.8-trace}, integrate against $\Lambda^Y_{\mathrm{sm}}$.
...
Smooth cutoff $\chi_\alpha$ bounds derivatives. Apply Lemma~B.\ref{lem:B-HS}.
\end{proof}
...
exactly as in Selberg’s formula. The geometric
...
\begin{itemize}
  ...
  \item Microlocal propagation bounds agree with semiclassical analysis frameworks.
  \item Stationary phase expansions are cross-checked against classical treatments.  
\end{itemize}
