...
% Full Monolithic Version: Sections A.0 to A.16
...
\begin{lemma}[Injectivity in truncated cusps]\label{appA:lem:inj-cusp}
...
\begin{proposition}[Geodesic collars near $\partial M(Y)$]\label{appA:prop:collar}
...
\begin{proposition}[Sectors based at the boundary]\label{appA:prop:sectors}
...
Lemma~\ref{appA:lem:balls}. The horocyclic boundary introduces only the global
factor $w_{\mathfrak a}/Y$ from \eqref{appA:eq:length-horo}.
\end{proof}

\subsection*{A.6. Effective comparison for $Y$ and $Y'$}
\label{appA:subsec:comparison}

\noindent
We will need to compare truncations at two heights $Y<Y'$.

\begin{lemma}[Difference of truncations]\label{appA:lem:Y-compare}
For $Y<Y'$,
\[
...
\begin{proof}
Subtract the identities in \eqref{appA:eq:vol-defect} for $Y$ and $Y'$.
\end{proof}

\begin{corollary}[Monotonicity and stability]\label{appA:cor:monotone}
...
\label{appA:subsec:smooth-truncation}
...
\begin{lemma}[Smoothed volumes]\label{appA:lem:smooth-vol}
...
\eqref{appA:eq:area-cusp}. The constants $\Psi_0,\Psi_1$ are finite by the support
...
\label{appA:subsec:spectral-interface}
...
\begin{lemma}[Plancherel-compatible normalization]\label{appA:lem:plancherel}
...
\begin{proof}
Immediate from Proposition~\ref{appA:prop:vol-defect} and Lemma~\ref{appA:lem:smooth-vol}.
\end{proof}

\begin{proposition}[Uniformity of geometric constants]\label{appA:prop:uniform-geom}
...
objects computed in Lemmas~\ref{appA:lem:ref-int}, \ref{appA:lem:smooth-vol}, 
and Propositions~\ref{appA:prop:vol-defect}, \ref{appA:prop:collar},
\ref{appA:prop:sectors}. None of these involve spectral parameters.
\end{proof}

\subsection*{A.9. Consistency checks and forward link}
\label{appA:subsec:consistency}

\noindent
\textbf{Consistency.}
Formulas \eqref{appA:eq:vol-defect} match verbatim the
...
\noindent
\textbf{Forward link.}
The identities of Lemma~\ref{appA:lem:plancherel} are invoked in Chapter~6 (identity
...
\label{appA:subsec:uniformity}

\begin{lemma}[Linear dependence on widths]\label{appA:lem:linear-w}
...
\begin{proof}
Immediate from Lemma~\ref{appA:lem:ref-int} and Proposition~\ref{appA:prop:vol-defect}.
Each cusp contributes independently and linearly in $w_{\mathfrak a}$.
\end{proof}

\begin{proposition}[Uniform Lipschitz bounds in $Y$]\label{appA:prop:lipschitz-Y}
Fix $\Gamma$. Then
\[
...
\begin{proof}
Differentiate the explicit formulas in Proposition~\ref{appA:prop:vol-defect}.
\end{proof}

\subsection*{A.11. Effective bounds in degenerating families}
\label{appA:subsec:families}

\begin{lemma}[Uniform bounds in cusp families]\label{appA:lem:family}
...
\begin{proof}
The first inequalities are the identities of Proposition~\ref{appA:prop:vol-defect}.
...
\label{appA:subsec:horocycles}

\begin{lemma}[Horocycle averages]\label{appA:lem:horo-av}
...
\label{appA:subsec:asymptotics}

\begin{proposition}[Asymptotic expansion of weighted tails]\label{appA:prop:asy-tail}
...
\eqref{appA:eq:ys-int}. The coefficients $c_j(s)$ arise from binomial expansions.
\end{proof}

\begin{corollary}[Stability of expansions]\label{appA:cor:stability-exp}
The expansions in Proposition~\ref{appA:prop:asy-tail} converge absolutely in the
...
\label{appA:subsec:sobolev}

\begin{lemma}[Weighted Sobolev inequality]\label{appA:lem:weighted-sob}
...
\label{appA:subsec:cross-checks}

\begin{itemize}
  \item \cite[§2]{Hejhal1983}: formulas for truncated cusp volumes match exactly
  \eqref{appA:eq:area-cusp}–\eqref{appA:eq:ys-int}.
  \item \cite[Chap.~3]{Buser1992}: the area of cusp collars agrees with our
  Proposition~\ref{appA:prop:collar}.
  \item \cite[§3]{Iwaniec2002}: normalizations of widths and horocycle lengths
  coincide with \eqref{appA:eq:length-horo}.
\end{itemize}

\subsection*{A.16. Audit of Appendix A}
\label{appA:subsec:audit}

\noindent
\textbf{Goals achieved.}
\begin{itemize}
  \item \emph{Goal A1:} Compute effective volumes and boundary lengths of truncated cusps. (Verified by Lemma~\ref{appA:lem:ref-int} and Proposition~\ref{appA:prop:vol-defect}).
  \item \emph{Goal A2:} Record weighted tail integrals. (Verified by Proposition~\ref{appA:prop:asy-tail}).
  \item \emph{Goal A3:} Establish injectivity radius and collar geometry. (Verified by Lemma~\ref{appA:lem:inj-cusp} and Proposition~\ref{appA:prop:collar}).
  \item \emph{Goal A4:} Provide smoothed truncation formulas. (Verified by Lemma~\ref{appA:lem:smooth-vol}).
  \item \emph{Goal A5:} Record dependence of constants on widths. (Verified by Lemma~\ref{appA:lem:linear-w}).
  \item \emph{Goal A6:} Supply asymptotic expansions and weighted Sobolev inequalities. (Verified by Proposition~\ref{appA:prop:asy-tail} and Lemma~\ref{appA:lem:weighted-sob}).
\end{itemize}

\noindent\textbf{Invariants.}
\begin{itemize}
  \item \emph{Invariant A1:} All constants are explicit and depend only on $\{w_{\mathfrak a}\}$, 
  smoothing profile $\psi$, and thick part data.
  \item \emph{Invariant A2:} No hidden dependence on $(\lambda,\eta)$.
  \item \emph{Invariant A3:} Normalizations match Hejhal, Buser, Iwaniec.
\end{itemize}

\noindent\textbf{Links.}
\begin{itemize}
  \item Forward: Sobolev constants (Ch. 2), Parabolic terms (Ch. 6), Weyl law (Ch. 8).
  \item Backward: Cusp width normalizations from Glossary.
\end{itemize}

\medskip
\noindent\textbf{Conclusion of Appendix A.}
Appendix~A has established a complete effective framework for geometric constants
on finite-area hyperbolic surfaces with cusps. These results form the geometric backbone
